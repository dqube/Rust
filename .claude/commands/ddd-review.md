...
2. Run `git diff` (staged + unstaged) and identify every file changed under `Src/crates/` and `Src/service/`.
3. For each changed file, check:
   ...
   - **Naming**: new crates and packages use the `ddd-` prefix. Service binaries live under `Src/service/`.
