...
//!
//! - **Edge** ([`edge`]) — `routes.yaml`-driven router, hyper service,
//!   accept loop, observability hooks, signal handling, `/metrics`
//!   server.
//! - **Transcode** ([`transcode`]) — descriptor-pool driven REST↔gRPC
//!   conversion, RFC 9457 `ProblemDetail`, SSE streaming.
...
//!   shared by the edge and the `/metrics` server.
...
//! proto descriptor pool. See the BFF wiring example in the README.
//!
//! ## Feature flags
//!
//! | Feature | Enables |
//! |---------|---------|
//! | `axum-response` | `axum::IntoResponse` for [`transcode::ProblemDetail`], the axum observability / tracing / audit middleware, the generic OpenAPI router + downstream spec merge, the HTTP [`proxy`], and the [`prelude`] module. Required for any axum-based REST layer. |
...
// Config
pub use config::{BffConfig, ResilienceConfig, env_or};
...
// Transcode — error types and pool helpers
pub use transcode::{
    app_error_to_problem, decode_pool, grpc_status_to_app_error, install, FieldViolation,
    ProblemDetail,
};
