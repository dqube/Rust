//! SeaORM implementation of [`ddd_shared_kernel::OutboxRepository`].

use async_trait::async_trait;
use chrono::{DateTime, Utc};
use ddd_shared_kernel::{AppError, AppResult, OutboxMessage, OutboxRepository};
use sea_orm::{
    ActiveModelTrait, ActiveValue::Set, ColumnTrait, DatabaseConnection, EntityTrait, QueryFilter,
    QueryOrder, QuerySelect,
};
use uuid::Uuid;

use crate::db::models::outbox::{ActiveModel, Column, Entity, Model};

/// Postgres-backed outbox repository.
pub struct SeaOrmOutboxRepository {
    db: DatabaseConnection,
}

impl SeaOrmOutboxRepository {
    /// Create a new repository.
    pub fn new(db: DatabaseConnection) -> Self {
        Self { db }
    }

    fn to_domain(m: Model) -> OutboxMessage {
        OutboxMessage {
            id: m.id,
            aggregate_id: m.aggregate_id,
            aggregate_type: m.aggregate_type,
            event_type: m.event_type,
            subject: m.subject,
            payload: m.payload,
            created_at: m.created_at,
            published_at: m.published_at,
            attempts: m.attempts.max(0) as u32,
            last_error: m.last_error,
        }
    }
}

fn db_err(e: impl std::fmt::Display) -> AppError {
    AppError::database(e.to_string())
}

#[async_trait]
impl OutboxRepository for SeaOrmOutboxRepository {
    async fn save(&self, message: &OutboxMessage) -> AppResult<()> {
        let am = ActiveModel {
            id: Set(message.id),
            aggregate_id: Set(message.aggregate_id.clone()),
            aggregate_type: Set(message.aggregate_type.clone()),
            event_type: Set(message.event_type.clone()),
            subject: Set(message.subject.clone()),
            payload: Set(message.payload.clone()),
            created_at: Set(message.created_at),
            published_at: Set(message.published_at),
            attempts: Set(message.attempts as i32),
            last_error: Set(message.last_error.clone()),
        };
        am.insert(&self.db).await.map_err(db_err)?;
        Ok(())
    }

    async fn mark_as_published(&self, id: Uuid) -> AppResult<()> {
        let found = Entity::find_by_id(id).one(&self.db).await.map_err(db_err)?;
        let Some(m) = found else { return Ok(()); };
        let mut am: ActiveModel = m.into();
        am.published_at = Set(Some(Utc::now()));
        am.last_error = Set(None);
        am.update(&self.db).await.map_err(db_err)?;
        Ok(())
    }

    async fn mark_as_failed(&self, id: Uuid, error: &str) -> AppResult<()> {
        let found = Entity::find_by_id(id).one(&self.db).await.map_err(db_err)?;
        let Some(m) = found else { return Ok(()); };
        let new_attempts = m.attempts + 1;
        let mut am: ActiveModel = m.into();
        am.attempts = Set(new_attempts);
        am.last_error = Set(Some(error.to_owned()));
        am.update(&self.db).await.map_err(db_err)?;
        Ok(())
    }

    async fn find_unpublished(&self, limit: u32) -> AppResult<Vec<OutboxMessage>> {
        let rows = Entity::find()
            .filter(Column::PublishedAt.is_null())
            .order_by_asc(Column::CreatedAt)
            .limit(u64::from(limit))
            .all(&self.db)
            .await
            .map_err(db_err)?;
        Ok(rows.into_iter().map(Self::to_domain).collect())
    }

    async fn delete_published_older_than(&self, older_than: DateTime<Utc>) -> AppResult<u64> {
        let res = Entity::delete_many()
            .filter(Column::PublishedAt.is_not_null())
            .filter(Column::PublishedAt.lt(older_than))
            .exec(&self.db)
            .await
            .map_err(db_err)?;
        Ok(res.rows_affected)
    }
}
